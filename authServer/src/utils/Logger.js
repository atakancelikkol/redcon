...
class Logger {
  constructor() {
    ...
    this.loggerLevel = 'debug'; // importance: error > warning > info > verbose > debug > silly
    this.loggerMaxFileSize = undefined;
    this.loggerCallerModuleDepth = 3;
  }
  ...
  transportSettings() {
    const transports = [];
    transports[0] = new winston.transports.Console({ level: this.loggerLevel });

    if (process.env.NODE_ENV !== 'test') {
      transports[1] = new winston.transports.File({
        filename: './logs/Server.log',
        maxsize: this.loggerMaxFileSize, // max size
  ...
  getCallSitesObject() {
    ...
    return callSites[this.loggerCallerModuleDepth];
  }
